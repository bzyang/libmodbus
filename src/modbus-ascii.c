...
{
	uint8_t high = nb >> 8;
	uint8_t low = nb & 0x00ff;
    assert(ctx->slave != -1);
	req[0] = 0x3a;
	req[1] = ASCII_H(ctx->slave);
    req[2] = ASCII_L(ctx->slave);
    req[3] = ASCII_H(function);
    req[4] = ASCII_L(function);
    req[5] = ASCII_H(addr>>8);
	req[6] = ASCII_L(addr>>8);
    req[7] = ASCII_H(addr&0x00ff);
    req[8] = ASCII_L(addr&0x00ff);
	req[9] = ASCII_H(high);
	req[10] = ASCII_L(high);
	req[11] = ASCII_H(low);
	req[12] = ASCII_L(low);
...
static int _modbus_ascii_build_response_basis(sft_t *sft, uint8_t *rsp)
{
	...
	rsp[0] = 0x3a;
	rsp[1] = ASCII_H(sft->slave);
	rsp[2] = ASCII_L(sft->slave);
	rsp[3] = ASCII_H(sft->function);
	rsp[4] = ASCII_L(sft->function);
...
static uint16_t lrc(uint8_t *buffer, uint16_t buffer_length)
{
	int sum=0x00;
	...
	{
		sum  = sum + ASCII2DEM(buffer , 0);
		buffer++;
		buffer++;
		buffer_length  =buffer_length - 2;
	}
	return 0xff+1-(sum-256*(sum/256));
}
...
{
	uint16_t ilrc = lrc(req+1, req_length-1); //0x3a not calculate
	//req[req_length++] = ASCII_H(ilrc);
	//req[req_length++] = ASCII_L(ilrc);
    uint8_t clrc = (uint8_t)ilrc;                           //modify by yangbingzhou
	req[req_length++] = ASCII_H(clrc);
	req[req_length++] = ASCII_L(clrc);
...
static void _modbus_ascii_ioctl_rts(int fd, int on)
{
    int flags;
...
static ssize_t _modbus_ascii_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    if (ctx_rtu->rts != MODBUS_RTU_RTS_NONE) {
        ...
        _modbus_ascii_ioctl_rts(ctx->s, ctx_rtu->rts == MODBUS_RTU_RTS_UP);
...
{
    uint16_t lrc_calculated;
    uint16_t lrc_received;
    int slave = msg[0];
...
//    crc_calculated = crc16(msg, msg_length - 2);
 //   crc_received = (msg[msg_length - 2] << 8) | msg[msg_length - 1];
		lrc_calculated =lrc(msg+1,msg_length-5);
		lrc_received =(msg[msg_length-4]<<4) | msg[msg_length-3];
    /* Check CRC of msg */
		if(ASCII_H(lrc_calculated)==msg[msg_length-4]&&ASCII_L(lrc_calculated)==msg[msg_length-3]){
			return msg_length;
		}
		else {
        if (ctx->debug) {
            fprintf(stderr, "ERROR LRC received %0X %0X != LRC calculated %0X %0X\n",
                    msg[msg_length-4],msg[msg_length-3],ASCII_H(lrc_calculated),ASCII_L(lrc_calculated));
        }
